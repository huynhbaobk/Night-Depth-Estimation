...
class Trainer:
    ...
    def predict_poses(self, inputs, features, syn_or_real="real"):
        ...
        if self.num_pose_frames == 2:
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    if self.opt.pose_model_type == "separate_resnet":
                        if syn_or_real=="real":
                            pose_inputs = [self.models["pose_encoder_real"](torch.cat(pose_inputs, 1))]
                        elif syn_or_real=="syn":
                            pose_inputs = [self.models["pose_encoder_syn"](torch.cat(pose_inputs, 1))]
                        
                    elif self.opt.pose_model_type == "posecnn":
                        pose_inputs = torch.cat(pose_inputs, 1)
                    if syn_or_real=="real":
                        axisangle, translation = self.models["pose_real"](pose_inputs)
                    elif syn_or_real=="syn":
                        axisangle, translation = self.models["pose_syn"](pose_inputs)
